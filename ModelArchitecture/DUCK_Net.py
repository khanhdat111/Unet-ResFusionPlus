...
from CustomLayers.ConvBlock2D import conv_block_2D, attention_fusion, upsample_to
...
def create_model(img_height, img_width, input_chanels, out_classes, starting_filters):
    ...
    print('Starting U-Net-ResFusion+')
    ...
    q4dc =  upsample_to(q4, z1)
    q3dc =  upsample_to(q3, z1)
    q6dc =  upsample_to(q6, z1)
    q1dc =  upsample_to(q1, z1)
    
    out_concatenate = Concatenate(axis=-1)([q4dc, q3dc, q6dc, q1dc, z1])
    out_concatenate = attention_fusion(out_concatenate, starting_filters)

    out_concatenate = conv_block_2D(out_concatenate, starting_filters, 'double_convolution', repeat=1)
        
    output = Conv2D(out_classes, (1, 1), activation='sigmoid')(out_concatenate)
